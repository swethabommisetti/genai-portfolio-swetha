...
import re
from utils.receipts_repo import upsert_receipts_dtl, ensure_receipt_file_id,save_receipt_and_items
...
def run_receipt_scanner():
    ...
    if uploaded:
        ...
        if not hasattr(upload_res, 'error'):
            ...
            try:
                insert_res = supabase.table("receipt_files").insert({
                    "filename": filename,
            ...
            if receipt_file_id is None:
                try:
                    sel = supabase.table("receipt_files").select("id").eq("filename", filename).limit(1).execute()
                ...
                with st.spinner("Running receipt extractor…"):
                    try:
                        ...
                        st.json(result if result is not None else {
                            "status": "ok",
                            "details": "No result payload returned."
                        })
                        ...
                        try:
                            ...
                            if rid is None:
                                st.warning("⚠️ Could not determine receipt_file_id; skipping receipts_dtl insert.")
                            else:
                                '''
                                upsert_receipts_dtl(
                                    supabase,
